            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item billing-nav-item">
                        <a class="nav-link {{ request()->routeIs('billing.*') ? 'active-nav' : '' }}" href="{{ route('billing.index') }}">
        ...
        function updateBillingBadge() {
            fetch('{{ route("billing.summary") }}')
                ...
                .then(data => {
                    ...
                    if (data.pending_bills > 0) {
                        badge.textContent = data.pending_bills;
                        badge.style.display = 'inline';
                    } else {
                        badge.style.display = 'none';
                    }
                })
                .catch(error => console.log('Billing update error:', error));
        }

        // Update billing badge on page load
        document.addEventListener('DOMContentLoaded', updateBillingBadge);

        // Update billing badge every 5 minutes
        setInterval(updateBillingBadge, 300000);
    </script>
    @endauth
